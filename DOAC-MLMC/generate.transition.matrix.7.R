...
age.independent.generate.probabilities<-function(n.samples)
{
  ...
  # Effect of prior Intracranial hemorrhage of future events
	...
	event.utilities[,"Transient ischemic attack (TIA)"]<-runif(n.samples,min=1.5*(-0.131),max=0.5*(-0.131))
	event.utilities[,"Clinically relevant bleeding"]<--rnorm(n.samples,-0.03,0.001531)
	event.utilities[,"SE"]<-runif(n.samples,min=1.5*(-0.131),max=0.5*(-0.131))
