EVPPI_NOAC_std_p_t<-function(M,N)
{
  ...
  sigma_coef <- sigma_loghr_t[nindex,index]%*%solve(sigma_loghr_t[index,index])
  sigma_cond <- sigma_loghr_t[nindex,nindex] - sigma_coef%*%sigma_loghr_t[index,nindex]
  
  mu_con <- matrix(mu_loghr_t[nindex],length(nindex),NN) + sigma_coef%*%(t(MCMC.loghr.samples[,index+7])-matrix(mu_loghr_t[index],length(index),NN))
  
  #d1_t <- t(t(MCMC.loghr.samples[,index+7])-matrix(mu_loghr[index],length(index),NN))%*%solve(sigma_loghr_t[index,index])%*%(t(MCMC.loghr.samples[,index+7])-matrix(mu_loghr[index],length(index),NN))
    
  #MCMC.loghr.samples[,nindex+7] <-  t(mu_con)+ rmvt(n=NN, sigma = sigma_cond, df = 19)
  for(i in 1:N){
    d1_t <- t(t(MCMC.loghr.samples[i,index+7])-mu_loghr_t[index])%*%solve(sigma_loghr_t[index,index])%*%(t(MCMC.loghr.samples[i,index+7])-mu_loghr_t[index])
    temp = as.numeric(d1_t)
    MCMC.loghr.samples[((i-1)*NN+1):(i*NN),nindex+7] <-   rmvt(n=NN, sigma = sigma_cond*(5+temp)/(5+14), delta=t(mu_con)[1,], df = 19)
  }
