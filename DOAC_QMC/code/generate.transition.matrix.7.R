...
rownames(hr.future.death)<-rownames(hr.future.bleed)<-rownames(hr.future.ich)<-
    rownames(hr.future.tiase)<-rownames(hr.future.stroke)<-hr.future.stroke[,1]
...
event.names<-c("MI","Ischemic stroke","Death (all causes)","Transient ischemic attack (TIA)",
               "Clinically relevant bleeding","SE","ICH","Stay")
...
" I + M + S "," B + I + M + S ")
treatment.names<-c("Coumarin (INR 2-3)","Apixaban (5mg bd)","Dabigatran (150mg bd)",
...
age.independent.generate.probabilities<-function(n.samples)
{
