...
EVPPI_HR_notreat_QMC_p<-function(N,M,K)
{
  inputs <- seq(1,K)
  ...
  results <- foreach(i=inputs,.export=c('bugs.baseline', 'bugs.loghr','hr.no.treatment','age.independent.generate.probabilities_2',
                                        ...
                                        'sort_baseline','sort_notreat'))  %dorng%  {
                                          ...
                                          U <- sobol(N, dim = 2, init = TRUE,  scrambling = 1, seed = 666, normal = FALSE)
                                          U <- sobol(N, dim = 2, init = FALSE, scrambling = 1, seed = 6, normal = FALSE)
                                          ...
                                          for(j in inputs) {
                                            ...
                                            temp = sort_notreat(U[index,])$QMCsample_notreat
