...
EVPPI_Switch_Prob_QMC_p<-function(N,M,K)
{
  inputs <- seq(1,K)
  ...
  results <- foreach(i=inputs,.export=c('bugs.baseline', 'bugs.loghr','hr.no.treatment','age.independent.generate.probabilities_2',
                                        ...
                                        'sort_baseline','sort_notreat'))  %dorng%  {
                                          ...
                                          U <- sobol(N, dim = 4, init = TRUE, scrambling = 1, seed = sample(seq(1,10^6),1), normal = FALSE)
                                          U <- sobol(N, dim = 4, init = FALSE, scrambling = 1, seed = sample(seq(1,10^6),1), normal = FALSE)
                                          ...
                                          for(j in inputs) {
                                            ...
                                            temp = Rbeta.inv(U[index,1], 0.1, 0.9, log=FALSE)
                                            Switch.probability.samples[,1] = rep(temp, times = 1, each = M)
                                            temp = Rbeta.inv(U[index,2], 0.1, 0.9, log=FALSE)
                                            Switch.probability.samples[,2] = rep(temp, times = 1, each = M)
                                            temp = Rbeta.inv(U[index,3], 0.1, 0.9, log=FALSE)
                                            Switch.probability.samples[,3] = rep(temp, times = 1, each = M)
                                            temp = Rbeta.inv(U[index,4], 0.3, 0.7, log=FALSE)
