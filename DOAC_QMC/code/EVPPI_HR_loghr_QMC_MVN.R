...
EVPPI_HR_loghr_QMC_MVN<-function(N,M,K)
{
  inputs <- seq(1,K)
  ...
  results <- foreach(i=inputs,.export=c('bugs.baseline', 'bugs.loghr','hr.no.treatment','age.independent.generate.probabilities_2',
                                        ...
                                        'sort_baseline','sort_notreat','mean_baseline','L_baseline','mean_loghr','L_loghr'))  %dorng%  {
                                          ...
                                          U <- sobol(N, dim = 28, init = TRUE,  scrambling = 1, seed = sample(seq(1,10^6),1), normal = FALSE)
                                          U <- sobol(N, dim = 28, init = FALSE, scrambling = 1, seed = sample(seq(1,10^6),1), normal = FALSE)
