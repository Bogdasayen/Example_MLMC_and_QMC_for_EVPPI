...
EVPPI_All_Utility_QMC_p<-function(N,M,K)
{
    inputs <- seq(1,K)
    ...
    results <- foreach(i=inputs,.export=c('bugs.baseline', 'bugs.loghr','hr.no.treatment','age.independent.generate.probabilities_2',
                                          ...
                                          'sort_baseline','sort_notreat'))  %dorng%  {
                                              ...
                                              U <- sobol(N, dim = 14, init = TRUE, scrambling = 1, seed = sample(seq(1,10^6),1), normal = FALSE)
                                              U <- sobol(N, dim = 14, init = FALSE, scrambling = 1, seed = sample(seq(1,10^6),1), normal = FALSE)
