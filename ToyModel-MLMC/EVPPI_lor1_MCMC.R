EVPPI_lor1_l_MCMC <- function(l, N) {
  require(Matrix)
  ...
  for(N1 in seq(1, N, by=10000)) {
    ...
    jags.object.recovery.update<-update(jags.object.recovery.fe,n.iter=ceiling(N2/2),progress.bar="none")
    jags.object.relapse.update<-update(jags.object.relapse.fe,n.iter=ceiling(N2/2),progress.bar="none")
    # get the matrix of the samples
    lor_rec <- jags.object.recovery.update$BUGSoutput$sims.matrix[1:N2,2:3]
    lor_rel <- jags.object.relapse.update$BUGSoutput$sims.matrix[1:N2,2:3]
