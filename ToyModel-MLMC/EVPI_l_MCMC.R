EVPI_l_MCMC <- function(l, N) {
  ...
  #  jags.samples(model=jags.object.recovery.fe, variable.names="d", n.iter=N2*M/2, thin = 1)
    jags.object.recovery.update<-update(jags.object.recovery.fe,n.iter=N2*M/2,progress.bar="none")
    jags.object.relapse.update<-update(jags.object.relapse.fe,n.iter=N2*M/2,progress.bar="none")
