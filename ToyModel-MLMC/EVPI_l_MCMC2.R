EVPI_l_MCMC2 <- function(l, N) {
  ...
  # run the mcmc chains and get the required number of samples
  #  jags.samples(model=jags.object.recovery.fe, variable.names="d", n.iter=N2*M/2, thin = 1)
  jags.object.recovery.update<-update(jags.object.recovery.fe,n.iter=N*M/2,progress.bar="none")
  jags.object.relapse.update<-update(jags.object.relapse.fe,n.iter=N*M/2,progress.bar="none")
  # get the matrix of the samples
  lor_rec <- jags.object.recovery.update$BUGSoutput$sims.matrix[,2:3]
  lor_rel <- jags.object.relapse.update$BUGSoutput$sims.matrix[,2:3]
  ...
  for(N1 in seq(1, N, by=10000)) {
    ...
    lor_t1_rec <- matrix(lor_rec[N1:(N1+N2-1),1],M,N2,byrow = TRUE)
    lor_t1_rel <- matrix(lor_rel[N1:(N1+N2-1),1],M,N2,byrow = TRUE)
